...
/* return the volume of a right rectangular prism with width w, length l,
 * and height h
 */
...
/* return the perimeter of the smallest face of a right rectangular prism with
 * width w, length l, and height h
 */
int smallest_face_perimeter(int w, int l, int h)
{
    return 2 * min(min(l+w, w+h), h+l);
}
...
int main()
{
    ...
    while (finput)
    {
        ...
        total += volume(output[0], output[1], output[2])
                     + smallest_face_perimeter(output[0], output[1], output[2]);
    }
